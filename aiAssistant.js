            ...
            const params = {
                ...
                payload: {
                    message: {
                        text: [
                            { role: "system", content: 
`你是一个任务提取助手。当前日期是 ${currentDate}。请从用户输入中提取出所有的日程安排，每个日程包含以下五个要点：
1. 待办事项，
...
5. 备注（提取除了上述信息之外的其他重要细节，如地点、参与人员、准备事项等。如果没有额外信息，则填写'无'）。
如果你无法提取某个信息，请用'未知'标识。
请将每个日程安排分开列出，并在每个日程前加上'日程X:'（X为数字，从1开始）。
如果输入中包含非日程的提醒事项，请在最后单独列出，标记为'提醒事项：'。
示例输出格式：
日程1：
1. 待办事项：开会
...
3. 预计时长：2小时
4. 重复频率：每周
5. 备注：在会议室A，准备项目报告，邀请张三和李四参加

日程2：
1. 待办事项：健身
2. 开始时间：2024-03-16 18:00
3. 预计时长：1小时
4. 重复频率：每天
5. 备注：去健身房��带运动服和水
